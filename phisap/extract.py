...
class Texture2D(Texture):
    ...
    def get_image(self) :
        if self.texture_format == self.TextureFormat.RGB24:
            return Image.frombytes('RGB', (self.width, self.height), self.image_data).transpose(1)
        elif self.texture_format == self.TextureFormat.RGBA32:
...
class FileReader(BinaryReader):
    ...
    def check_file_type(self) -> FileType:
        ...
        self.pos = 0
        if signature in [b'UnityWeb' , b'UnityRaw' , b'UnityArchive' , b'UnityFS']:
            return self.FileType.BundleFile
        if signature == b'UnityWebData1.0':
...
class AssetsManager:
    ...
    def load_bundle(self, reader: FileReader, original_path=''):
        ...
        for asset_file in self.asset_files:
            for object_info in asset_file.object_infos:
                with ObjectReader(asset_file.reader, asset_file, object_info) as obj_reader:
                    obj = None
                    t = obj_reader.class_id
                    if t == ClassID.TEXT_ASSET:
                        obj = TextAsset(obj_reader)
                    elif t == ClassID.TEXTURE_2D:
...
if __name__ == '__main__':
    apk_file = zipfile.ZipFile('base.apk')
    catalog = Catalog(apk_file.open('assets/aa/catalog.json'))
    with open("catalog.json","w") as f:
        import json
        json.dump(catalog.fname_map,f)
