...
function CreateChartInfoSprites(chartInfo, sprite, pixi, stage, requireFPSCounter = false, noStartAnimate = false) {
    ...
    // 创建水印
    if (!output.watermark) {
        let watermark = new PIXI.Text('Ph1gr0s Emulator Remixed by lucky', {
            fontFamily: 'Mina',
            fill: 'rgba(255, 255, 255, 0.5)',
            align: 'right'
        });
...
function CreateGameEndAnimation(pixi, sprites) {
...

    {
        ...
        switch (levelDiffType) {
            case 'ez':
                {
                    audioTexture = textures.sound.levelOver.ez;
                    break;
                }
            case 'hd':
                {
                    audioTexture = textures.sound.levelOver.hd;
                    break;
                }
            case 'in':
                {
                    audioTexture = textures.sound.levelOver.in;
                    break;
                }
            case 'at':
                {
                    audioTexture = textures.sound.levelOver.at;
                    break;
                }
            case 'sp':
                {
                    audioTexture = textures.sound.levelOver.sp;
                    break;
                }
            default:
                { audioTexture = textures.sound.levelOver.in; }
...
function CreateClickAnimation(offsetX, offsetY, angle, score, performance = false) {
    ...
    let the_offset_of_the_offset = pixi.renderer.realWidth - fixedWidth
    the_offset_of_the_offset /= 2
    offsetX -= the_offset_of_the_offset // 你大爷的，我给你加了个偏移
...
function PlayHitsound(note, volume) {
    ...
    if (note.hitsound) {
        ...
        switch (note.type) {
            case 1:
                {
                    hitsoundTexture = textures.sound.tap;
                    break;
                }
            case 2:
                {
                    hitsoundTexture = textures.sound.drag;
                    break;
                }
            case 3:
                {
            ...
            case 4:
                {
                    hitsoundTexture = textures.sound.flick;
                    break;
                }
            default:
                { hitsoundTexture = textures.sound.tap; }
